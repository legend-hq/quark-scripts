...
library Actions {
    ...
    function constructBridgeOperations(
        ...
        // TODO: NEED SOME LOGIC TO HANDLE WETH/ETH IN ACROSS. ALSO TO BRIDGE THE COUNTERPART TOKEN AS WELL
    ...
    {
        // TODO: bridge using Across for USDC as well
        if (Strings.stringEqIgnoreCase(bridge.assetSymbol, "USDC")) {
            return bridgeUSDC(bridge, payment, useQuotecall);
        }
        // TODO: do WETH only for now
        else if (
            Strings.stringEqIgnoreCase(bridge.assetSymbol, "WETH")
                || Strings.stringEqIgnoreCase(bridge.assetSymbol, "ETH")
        ) {
        ...
        returns (IQuarkWallet.QuarkOperation memory, Action memory)
    {
        ...
        // Decode the result
        uint256 totalRelayFeePct = abi.decode(result, (uint256));

        // TODO: maybe add some buffer to this
        uint256 inputAmount = bridge.amount * 1e18 / (1e18 - totalRelayFeePct);
        console.logUint(bridge.amount);
        ...
        IQuarkWallet.QuarkOperation memory quarkOperation = IQuarkWallet.QuarkOperation({
            ...
            isReplayable: false,
            scriptAddress: CodeJarHelper.getCodeAddress(scriptSources[0]),
            // TODO: INPUT AMOUNT NEEDS TO BE DETERMINED VIA FFI (bridge.amount)
